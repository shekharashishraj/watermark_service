...
const WatermarkContent = styled(motion.div)`
  ...
  background: ${props => props.isSelected ? 'rgba(255,255,255,0.9)' : 'rgba(102, 126, 234, 0.1)'};
  border: 2px solid ${props => props.isSelected ? '#667eea' : 'rgba(102, 126, 234, 0.3)'};
...
function SimplePDFViewer({ 
  ...
  return (
    <ViewerContainer ref={containerRef}>
        ...
        >
          {pdfUrl ? (
            <PDFPreview 
              ...
              title="PDF Preview"
              onError={() => {
                // Show fallback if PDF can't load
            ...
            <PDFPlaceholder>
              ...
              <h3>Interactive PDF Watermarking Studio</h3>
      ...
      <StatusBar>
        <div>
          Interactive PDF Studio | {watermarks.length} watermarks
        </div>
        <div>
