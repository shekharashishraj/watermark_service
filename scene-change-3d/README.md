...
| Detection | `detect.py` | Missing: baseline surfaces seen through (free-space violation). Added: new surfaces where the baseline saw free space. Moved: missing/added pairs matched by shape and colour. Restyled: colour change after a per-frame colour fit, with texture-aware thresholds and cross-view agreement. Anything never observed again is reported as unverified, per room. Capture-quality safeguards (from the robustness study): a clean verdict needs at least 80% of the depth to agree with the baseline, and clipped or crushed pixels and frames the colour fit cannot model give no appearance evidence; otherwise the verdict says "Not verified (poor depth / poor exposure)". |
